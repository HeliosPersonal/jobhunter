...
| AC-12 | `FactuallyExcludedJob_IsRecordedSuppressed_CountedInFooter_AndNotDeepMatched` | Integration |
| AC-13 | `MatchAllJobsCalibrationPass_BypassesPreMatchFilter_AndMatchesEverything` | Integration |
...
## The pre-match reference corpus

`Data/pre-match-corpus.yaml` — the labelled set against which the pre-match filter's pass rate is
asserted (T12, AC-12). It is a fixed set of **enriched jobs paired with the active `Profile`**, each
labelled with the expected outcome — `pass` (reaches deep tier) or `exclude` (fails a factual
disqualifier) — and, for excluded jobs, the **rule** that excludes it (timezone, employment type,
seniority floor, salary floor, lifecycle). Each `exclude` label names the specific rule so the test
verifies the *reason* recorded, not merely the count.

The corpus is sized and balanced so the expected pass fraction lands squarely inside the **35–50%**
band (NFR): a job labelled `exclude` must fail exactly one named factual rule; a job labelled `pass`
must clear all five. The corpus grows by defect — a pre-match filter regression that ever passed or
excluded the wrong job is added as a labelled case before the fix merges. It is distinct from the
golden ranking set (which measures the *deep-tier* judgement); this corpus measures only the
*pre-match* factual gate and is judged with no LLM in the loop.
...
- `Data/pre-match-corpus.yaml` — the labelled pre-match reference corpus above.
...
- **precision@10** → not CI-assertable. Measured weekly from Owner ratings and tracked; the golden
  set is the CI proxy.
- Matching cost < $0.60 → asserted against `PricingTable` over 150 golden-set-sized items (no hard-coded dollar literal; ≈$0.44 typical).
