...
| Method | Path | Scope | Purpose |
|---|---|---|---|
| `GET` | `/api/cv` | `jobhunter:read` | Active CV version, activation date, matches computed against it — **metadata only** |
| `POST` | `/api/cv` | `jobhunter:read` | Upload a new CV version; re-stales existing matches and queues a re-match |

Both CV endpoints are **owner-scoped**: `jobhunter:read` **plus** the `sub` == Owner check (see
[[../../../engineering/security|security]] §Scope glossary). The CV is the single most sensitive
artifact — a valid `jobhunter:read` token for any other subject is a 403. `GET /api/cv` returns only
metadata (version, `activatedAt`, match count); **it never returns CV content**, the one boundary the
CV crosses being the F4 match prompt ([[../../f4-cv-matching-ranking/contracts/match-schema|F4 CV
handling rules]]). `POST /api/cv` activates a new immutable version, which re-stales existing matches
and queues a re-match of the last 30 days
([[../../f4-cv-matching-ranking/adr/0002-cv-versioning-and-restaling|ADR-F4-0002]]). These endpoints
back F4 AC-06/AC-07 and are exercised by F4 T03.
...
| `POST` | `/api/runs` | `jobhunter:admin` | Start a Run now; refused if one is already live |
| `POST` | `/api/runs/{id}/abort` | `jobhunter:admin` | Abort a non-terminal run |
...
`POST /api/runs` starts a Run immediately; there is at most one live Run, so it is refused with a
`409` if one is already in flight ([[../../f3-claude-batch-enrichment/data-model|F3 data model]]). It
backs F3 AC-12. `POST /api/runs/{id}/abort` transitions a non-terminal Run to `CostAborted`; a
terminal Run is refused with a `409`. `redeliver` is safe by construction: the delivery log means
already-sent cards are not sent again
