...
`JobHunter.ArchitectureTests` asserting every rule in
[[../../../engineering/coding-standards|standards]] §2 — dependency direction, `Contracts` referencing
nothing, Dapper never writing, no ambient clock, AppHost isolation, `internal sealed` entity
configurations, no stray `public` types in Infrastructure. Each test ships with a deliberately
violating fixture in an excluded `~Violations` folder.
