...
**Given** an opportunity whose application destination is **confirmed unreachable** (a definitive
4xx/5xx response or a DNS failure)
**When** the digest is assembled
**Then** it is not presented as an actionable card. A destination whose verification only **timed
out**, or which could not be checked because `robots.txt` disallows it, is treated as **unverified**,
not unreachable: it is still presented, flagged "link unverified".
...
| Apply-link liveness | 0 presented cards confirmed-unreachable; unverified (timeout / robots) cards flagged | Pre-delivery check |
