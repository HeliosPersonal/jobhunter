...
JobHunter.Application/Reporting/  DigestAssembler · ApplyLinkVerifier · NearDuplicateGrouper
...
sequenceDiagram
  ...
  A->>A: NearDuplicateGrouper — group same-opening cards for display (ADR-F2-0001)
  A->>V: verify apply destinations (HEAD, 5 s timeout, bounded parallelism)
  V-->>A: reachable / confirmed-unreachable / unverified (timeout, robots-disallowed)
  A->>A: drop confirmed-unreachable cards; keep unverified with a "link unverified" flag (AC-11)
...
| D3 | Apply-link verification adds latency and can produce false negatives | A live job dropped from the digest | 5 s timeout, bounded parallelism. Only a **confirmed-unreachable** link (4xx/5xx/DNS) drops a card (AC-11). A **timeout is unverified, not unreachable** — the card is kept and flagged "link unverified"; likewise a robots-disallowed apply URL is unverifiable, so it is shown with the same flag. Timeout ≠ confirmed-unreachable |
