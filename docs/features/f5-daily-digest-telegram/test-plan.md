...
| AC-11 | `ConfirmedUnreachableApplyDestination_IsNotPresented` · `TimedOutOrRobotsDisallowedLink_IsPresentedWithUnverifiedFlag` | Integration |
...
- Apply-link liveness → integration test with a fake HTTP layer returning 200 (reachable, presented),
  404/410/5xx and DNS failure (confirmed-unreachable, suppressed), and a timeout plus a
  robots-disallowed URL (unverified, presented with the "link unverified" flag).
