...
| T03 | [[T03-dispatcher\|Dispatcher: allowlist, resolution, scope, rate limit]] | telegram | T02 | M | pending |
...
- **DoD (every task):** code compiles with zero warnings; the conformance suite is green in both directions; every command declares a scope; output goes through F5's formatter; the coverage gate stays green; the tracker row is updated in the same PR.
