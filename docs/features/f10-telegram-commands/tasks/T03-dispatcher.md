# T03 — Dispatcher: allowlist, resolution, scope, rate limit

**Layer:** telegram · **Deps:** T02 · **Est:** M · **Owner:** Viacheslav

## What

`CommandDispatcher`: allowlist before anything else, resolve against the registry, check
scope, apply the per-chat rate limit, invoke. The ordering matters — an unauthorised chat must be
dropped before resolution, so probing reveals nothing about the catalogue.

## Done when

- An unauthorised chat is dropped before resolution and receives **nothing**; only a log line is written (AC-10).
- Scope is checked from the descriptor; there is no path to a handler that bypasses it.
