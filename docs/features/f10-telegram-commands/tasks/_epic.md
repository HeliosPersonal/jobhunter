...
Twenty-two commands that make the whole system reachable from the chat: search the corpus, work the
...
surface-shaped: 22 commands drift apart unless something holds them together. That something is
...
- Contract: [[../contracts/command-catalogue|command catalogue]] — all 22 commands
...
**In:** the command registry and argument parser, dispatch with allowlist/capability/rate-limiting,
conversation state and cancellation, the confirmation flow, all 22 command handlers, menu
...
See [[tracker|tracker]]. 10 tasks, ≈ 5.5 person-days.
...
- Every command declares a capability; every state-changing command requires a confirmation naming
  its effect. Both assertions have proven-red fixtures.
