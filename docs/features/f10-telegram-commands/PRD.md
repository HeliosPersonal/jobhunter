...
The thing to guard against is scope: a bot with twenty commands drifts toward being a chat interface,
...
**so that** I do not have to remember twenty commands.
...
- **Abuse cases:**
  ...
  - A new command shipped without a scope → the registry convention test fails the build (AC-12).
