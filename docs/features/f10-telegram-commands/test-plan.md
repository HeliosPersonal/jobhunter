...
> keeps twenty commands from drifting apart.
...
| **Conformance** | Registry ↔ menu ↔ help ↔ contract document bijection; scope declared on every command | No | Reflection over the registry + markdown parse |
...
| AC-12 | `EveryCommand_DeclaresScopeAndStateChange` | **Conformance** |
...
4. **Registry → safety.** Every descriptor declares a scope; every `ChangesState` descriptor has a
   confirmation path. A fixture violating each proves both can go red.
