...
| QG-2 | **A command cannot ship unsafe** — undeclared scope or undeclared state-change fails the build | Registry convention test with a proven-red fixture |
...
| # | Choice | Why |
|---|---|---|
| S1 | **A declarative registry**: each command is a record with name, description, arguments, scope, state-changing flag and handler | One place defines the surface; the menu, help, authorization and docs all derive from it ([[adr/0001-declarative-command-registry\|ADR-F10-0001]]) |
...
| S3 | Multi-step commands hold a short-lived per-chat state with a 5-minute timeout | `/note` with no text, `/cv` awaiting a document. Bounded so a forgotten command never wedges the chat (AC-08) |
...
```text
JobHunter.Domain/Commands/       CommandDescriptor · CommandScope · ArgumentSpec
...
public sealed record CommandDescriptor(
    ...
    CommandScope Scope,                // Owner | Operator
...
- **When:** a descriptor omits its scope, or a state-changing command omits its confirmation.
...
| # | Item | Impact | Plan |
|---|---|---|---|
| D1 | Twenty commands is a lot to remember | The surface goes unused | Generated client menu, `/help` grouped by theme, edit-distance suggestions; per-command usage metrics flag dead commands for removal |
