...
- The surface will grow. Twenty commands today, more when a feature lands; the mechanism has to make
  growth safe rather than rely on the author remembering four places to edit.
- Authorization must fail closed. A command reachable without a declared scope is the one bug in this
  feature that actually matters.
- [[../../../CONTEXT]] invariant 12 and the F5 boundary mean some commands are more dangerous than
  they look — `/run` spends money, `/cv` triggers a re-match.
...
Each command is one `CommandDescriptor` record: name, summary, argument spec, scope, `ChangesState`,
handler, and a `ContractAnchor` naming its heading in
...
| Authorization | The dispatcher reads `Scope` before invoking; there is no other path to a handler |
...
- A command cannot ship without a scope or without documentation; both fail the build.
- The whole surface is readable in one file, which is what makes review possible at twenty commands.
