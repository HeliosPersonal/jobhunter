...
**Legend.** *Capability* — `Standard` (an everyday read/action) or `Sensitive` (touches system state
or is destructive; gated behind an extra confirmation). `CommandCapability` is a per-command
sensitivity flag, **not** a second identity — the Owner is the sole principal (invariant 9). *State* —
✎ means it changes state and therefore requires a confirmation tap ([[../sad|SAD]] §6.3).
...
### `/digest`
**Capability** Standard · **State** read · **Args** none
...
### `/more [count]`
**Capability** Standard · **State** read · **Args** `count` — optional, 1–20, default 5
...
### `/search <query>`
**Capability** Standard · **State** read · **Args** free text with optional inline filters
...
### `/note [text]`
**Capability** Standard · **State** ✎ · **Args** free text, optional
...
This week's outcomes — delivered, opened, ignored, applied — and the `precision@10` trend. Owned by
[[../../f5-daily-digest-telegram/tasks/T11-command-set|F5 T11]] (handler lives there); F10 registers
and catalogues it ([[../../f5-daily-digest-telegram/contracts/telegram-messages|F5 message contract]]).
...
**Capability** Standard · **State** read · **Args** company name or canonical domain
...
**Capability** Standard · **State** ✎ · **Args** company name or domain
...
**Read-only metadata.** Shows the active CV version, when it was activated, and how many current
matches were computed against it. **Never shows CV content, and never uploads** — the F4 boundary
holds here ([[../../f4-cv-matching-ranking/contracts/match-schema|F4 CV handling rules]]). Uploading a
new CV is done through F4's upload service outside the command surface; `/cv` only reports status
([[../sad|SAD]] §2).
...
### `/forget <dimension>`
**Capability** Standard · **State** ✎ · **Args** dimension name, or nothing to pick from a list
...
**Capability** Standard · **State** ✎ · **Args** amount, optional ISO currency (default EUR)
...
### `/status`
**Capability** Sensitive · **State** read · **Args** none
...
### `/cost [month]`
**Capability** Sensitive · **State** read · **Args** optional `YYYY-MM`, default current
...
### `/run`
**Capability** Sensitive · **State** ✎ · **Args** none
...
### `/help [command]`
**Capability** Standard · **State** read · **Args** optional command name
...
Each command has **exactly one owning feature** that provides the handler; every other feature that
"ships" it instead **registers against F10's registry**. F10 owns the registry and this catalogue.

| Command(s) | Sole owner |
|---|---|
| `/start`, `/help`, `/digest` | **F5 T11** — the bootstrap subset that ships with the first digest |
| `/saved`, `/stats` | **F5** |
| `/pipeline`, `/note` | **F6** (F5/F10 register, do not implement) |
| `/search` | **F9** (F5/F10 register) |
| `/hidden` | **F7** (F7 T08 keeps the handler; F10 registers) |
...
([[../sad|SAD]] §4 S5). `Sensitive` commands are included — there is one Owner, and hiding them
