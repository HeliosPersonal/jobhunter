...
- F2 reads `raw_postings` and writes `jobs`. Its only write to an F1-owned table is the single
  whitelisted **delete** — the 90-day retention prune of `raw_postings` (T09), an intentional seam
  acknowledged by F1 ([[../../architecture/data-model]] §1). It writes no other F1 table.
...
fingerprinting, alias recording, lifecycle and closure, reprocessing.

**Out:** enrichment, matching, ranking, search indexing, translation, and **near-duplicate
display grouping** — which is computed at digest assembly in F5 ([[adr/0001-conservative-fingerprint\|ADR-F2-0001]]).
...
| S4 | Near-duplicates grouped for display, never merged — grouping computed at digest assembly (F5) | QG-1. Grouping is reversible; merging is not ([[adr/0001-conservative-fingerprint\|ADR-F2-0001]]) |
...
JobHunter.Application/Deduplication/   DeduplicationHandler · FingerprintCalculator
                                       JobLifecycleService
                                       (NearDuplicateGrouper relocated to F5 digest assembly — ADR-F2-0001)
