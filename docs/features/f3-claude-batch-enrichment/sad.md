...
stateDiagram-v2
  ...
  Matching --> Failed
  Reporting --> Failed
  CostAborted --> Reporting : reduced digest still delivered
  Delivered --> [*]
  Failed --> [*]
```
...
Note that `CostAborted` flows into `Reporting`, not to a terminal stop: hitting the ceiling still
produces a digest, with a visible warning. Silence would be worse than a reduced digest.
