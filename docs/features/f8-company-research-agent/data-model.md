...
> **References (do not redefine):** `companies` (F1 — F8 updates `stage`), `runs` (F3).
...
- **Updates** `companies.stage` from the funding category (AC-10) — the one cross-feature write in F8,
  and it is deliberate: better data should improve ranking, not only the dossier.
