...
A table can be enumerated by a test, which is how the transition matrix suite covers all **49 pairs**
(7 statuses × 7, including self-transitions) rather than the handful someone thought of.

The seven states and their legal moves — the same table the contract enumerates and the diagonal
self-transitions the suite exercises:

```mermaid
stateDiagram-v2
  [*] --> New : first action (lazy create)
  New --> Saved
  New --> Applied
  New --> Interview
  New --> Rejected
  ...
  Saved --> Rejected
  Saved --> Ignored
  Applied --> Saved : correction
  Applied --> Interview
  Applied --> Rejected
  Applied --> Offer
  Applied --> Ignored
  Interview --> Interview : further rounds
  Interview --> Rejected
  Interview --> Offer
  Interview --> Ignored
  Rejected --> Applied : role re-opened
  Rejected --> Ignored
...
Self-transitions (the legal no-ops on the diagonal, e.g. `Applied → Applied`) are omitted from the
diagram for readability but are part of the 49-pair table; only `Interview → Interview` is shown
because it carries meaning (a further round).
