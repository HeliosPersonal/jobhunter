...
> The centrepiece is the **transition matrix suite** — all 49 status pairs (7×7, including
> self-transitions) enumerated, not the ones someone remembered.
...
| **Matrix** | All 49 `(from, to)` pairs against the contract table | No | xUnit theory over the enum product |
...
## The never-applies guard (invariant 7)

A negative test asserts the feature has **no apply path at all** — the system records that the Owner
applied, it never applies for them ([[../../CONTEXT]] invariant 7).

- `NoApplyPath_ExistsInApplicationTracking` — a reflection/architecture assertion that no type in
  `Domain/Applications` or `Application/Applications` submits a form, sends an email, or performs an
  outbound HTTP POST to a job board. Setting `Applied` is a **status the Owner chooses**; the only
  effect is a transition row and a signal, never an external action.
- `MarkApplied_ProducesTransitionAndSignalOnly_NoOutboundCall` — asserts that advancing to `Applied`
  writes exactly one transition and one signal and makes **zero** outbound calls (the notifier and any
  HTTP client are substitutes and must record no invocation).
