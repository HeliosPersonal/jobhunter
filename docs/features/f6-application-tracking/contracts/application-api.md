...
Rows are the current status, columns the target. `✓` permitted, `—` refused. Seven statuses give a
full **7×7 grid**, so every `(from, to)` pair — including the diagonal and every `→ New` cell — is
defined rather than left implicit.

| From \ To | New | Saved | Applied | Interview | Rejected | Offer | Ignored |
|---|---|---|---|---|---|---|---|
| **New** | ✓ | ✓ | ✓ | ✓ | ✓ | — | ✓ |
| **Saved** | — | ✓ | ✓ | ✓ | ✓ | — | ✓ |
| **Applied** | — | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
- **The diagonal is permitted** as a legal no-op: `Applied → Applied` is a re-affirmation (but
  `applied_at` is set only once), and `Interview → Interview` records a second round — the transition
  record is how the rounds are visible. A self-transition never fails; it is the safe idempotent case.
- **The `New` column is refused for every source but `New` itself.** `New` is the lazily-created entry
  state; there is no legitimate move *back* to "not yet acted on", so `X → New` is impossible and the
  remedy names the correct forward move.
...
The matrix is enumerated by the test suite over the Cartesian product `ApplicationStatus ×
ApplicationStatus`, so all **49 pairs** (7×7, including self-transitions) are covered rather than the
handful someone thought of.
