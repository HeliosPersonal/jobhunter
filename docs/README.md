...
| [[ARCHITECTURE-OPEN-DECISIONS]] | Open decisions ranked by blast radius — only O2 and O5 remain blocking; the rest are decided and closed |
| [[IMPLEMENTATION-READINESS]] | The gate between documentation and code: artifact matrix, ten build gates, build order |
| [[SDLC-AUDIT]] | **Independent review of that gate (2026-08-02).** 31 defects, 9 blocking; all resolved — see its §0 Resolution Log. Post-remediation verdict: APPROVED WITH CONDITIONS |
| [[AUDIT-RESOLUTION-DECISIONS]] | **The authoritative resolution of that audit** — every contested value decided, the source of truth when any other document disagrees |
| [[AUDIT-RESOLUTION-TRACKER]] | **The parallel implementation plan** — 125 tasks grouped into waves and non-conflicting lanes so multiple engineers/agents work concurrently without contention |
...
|---|---|---|---|---|
| F0 | [[features/f0-platform-foundation/index\|Platform foundation]] | M1 | 16 | [[features/f0-platform-foundation/tasks/tracker\|tracker]] |
| F1 | [[features/f1-ats-job-discovery/index\|ATS job discovery]] | M2 | 13 | [[features/f1-ats-job-discovery/tasks/tracker\|tracker]] |
...
| F3 | [[features/f3-claude-batch-enrichment/index\|Claude batch enrichment]] | M3 | 14 | [[features/f3-claude-batch-enrichment/tasks/tracker\|tracker]] |
| F4 | [[features/f4-cv-matching-ranking/index\|CV matching & ranking]] | M3 | 13 | [[features/f4-cv-matching-ranking/tasks/tracker\|tracker]] |
| F5 | [[features/f5-daily-digest-telegram/index\|Daily digest & Telegram]] | **M4** | 13 | [[features/f5-daily-digest-telegram/tasks/tracker\|tracker]] |
...
**125 tasks across 11 features.** Build order and dependencies: [[IMPLEMENTATION-READINESS]] §3.

> **Task-count reconciliation (finalised).** The counts above are the post-audit-remediation totals.
> The SDLC-audit remediation ([[AUDIT-RESOLUTION-DECISIONS]] §17) added: F0 T15 (backup job) and
> F0 T16 (`replay-dlq` CLI); F1 T13 (`JobClosed` publisher); F3 T14 (Ollama fallback adapter, numbered
> T14 to avoid a collision with the existing T13); F9's CV endpoints and Run start/abort folded into
> its existing T05/T06. F2's near-duplicate grouping moved from F2 to F5 T13 at digest assembly
> (ADR-F2-0001). F4 was corrected from 11 to 13 for the two ADR-F4-0003 optimisation tasks. Grand
> total re-summed against the filesystem: 16+13+9+14+13+13+9+9+9+10+10 = **125**.
...
| [[features/f10-telegram-commands/contracts/command-catalogue\|Command catalogue]] | F10 — all 22 commands, arguments, output shapes |
