...
> The authoritative schema for `{env}_jobhunter`. Per-feature `data-model.md` files refine the
> tables they **own** and must not redefine tables owned elsewhere.
>
...
| Preferences | `signals`, `preference_models`, `preference_weights`, `suppressions` | [[../features/f7-preference-learning/index\|F7]] |
| Research | `company_research`, `research_claims` | [[../features/f8-company-research-agent/index\|F8]] |
...
inserts nothing and only bumps `job_sources.last_seen_at`.
...
| `employment_type` | text | NOT NULL | `FullTime`, `Contract`, `PartTime`, `Unknown` |
...
one row per item so a malformed item is isolated, retained and retryable.
...
`digests`: `(id, run_id UNIQUE, total_new_jobs, excellent_matches, avg_salary_usd, market_note text,
narrative text, generated_at)`.

`digest_cards`: `(id, digest_id, job_id, rank smallint, score numeric(5,2), reasons jsonb, card_key text)`.
Unique `(digest_id, job_id)`.
...
### `company_research` / `research_claims` — F8

`company_research`: `(id, company_id, run_id, summary text, generated_at, prompt_version)`.
`research_claims`: `(id, research_id, category text, claim text, source_url text NOT NULL, observed_at)`
where `category` ∈ `Funding`, `EngineeringBlog`, `OpenSource`, `Reviews`, `News`, `Layoffs`, `Stack`,
`InterviewProcess`. **`source_url NOT NULL` is invariant 5.**
...
- **F7** creates `signals`, `preference_models`, `preference_weights`, `suppressions`.
- **F8** creates `company_research`, `research_claims`.
- **F9** creates no tables — it reads, and projects into Typesense.

Each feature's migration is additive. No feature alters a table it does not own; a needed change to
someone else's table is a task in *that* feature.
