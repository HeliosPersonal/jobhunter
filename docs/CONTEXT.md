...
| **Run** | One end-to-end execution of the daily pipeline, from discovery cut-off to delivered Digest. Has an id, a state machine, a cost ledger, and is resumable. | Not a Batch. One Run submits several Batches. |
...
| **ApplicationStatus** | `New → Saved → Applied → Interview → Rejected \| Offer`. A closed enum with a legal-transition table. | Not free text. |
| **Signal** | One recorded Owner action carrying preference information (`Ignored`, `Saved`, `Applied`, `Opened`, `Dismissed`) with the Job facts at the moment of the action. | Not a Match. Signals are *evidence*; the Preference model is the *conclusion*. |
...
9. **Single Owner.** No registration, no tenant column, no role model. Authorisation is a Keycloak subject allowlist for the API and a Telegram chat-id allowlist for the bot.
