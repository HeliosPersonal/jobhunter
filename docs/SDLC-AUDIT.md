...
>
> **Original verdict: REJECTED** — 31 defects, 9 blocking. Remediation plan in §13 and §15.
>
> **Post-remediation verdict: ✅ APPROVED WITH CONDITIONS.** All 31 defects (C-1…C-31, D-21…D-30) have
> been resolved in the documentation. Every contested value is now pinned in
> [[AUDIT-RESOLUTION-DECISIONS]] (the single source of truth), and the feature documents have been
> corrected to match. The parallel work-breakdown the audit found missing (R-19, §11-17) is
> [[AUDIT-RESOLUTION-TRACKER]]. See **§0 Resolution Log** below for the defect-by-defect closure.

---

## 0. Resolution Log

Every defect from this audit has been closed. The canonical decisions live in
[[AUDIT-RESOLUTION-DECISIONS]]; this log maps each blocker to where it was fixed.

### Critical (C-1…C-9) — all resolved

| # | Resolution | Decisions § |
|---|---|---|
| C-1 | Open-decision register reconciled: O1/O3/O4/O6/O8/O10/O12 closed as ADR-resolved; O2→F9 T04, O5→F7 T07 kept open with corrected pointers; readiness gate re-run **per task**; BACKLOG §6 lists only O2/O5 | §7 |
| C-2 | `architecture/data-model.md` demoted to a **derived reconciled roll-up**; feature models are normative; ~16 tables reconciled; invariant 5 uses F8's FK encoding | §1, §5 |
| C-3 | `CONTEXT.md` amended: `ApplicationStatus` 7 (+`Ignored`), `Signal.kind` 8 (−`Dismissed`), `Run.state` 9 (no `Discovering`), `CommandScope`→`CommandCapability {Standard, Sensitive}` | §2 |
| C-4 | F6 transition matrix rebuilt to full **7×7=49** grid with the `New` column present; count settled to 49 in all four docs; `Application` state diagram added | §4 |
| C-5 | ADR-F4-0003 fully integrated: AC-12/13 test rows, reference corpus defined, SAD §9 entry, ERD cardinality fixed, cache-control constraint in `match-schema.md`, filter-vs-suppression precedence stated, tracker header corrected | §3, §6, §17, §18 |
| C-6 | Backup task added: **F0 T15** (nightly `pg_dump` → Azure Blob); runbook R9 points to it | §17 |
| C-7 | CV endpoints (`GET`/`POST /api/cv`) and Run start/abort (`POST /api/runs`, `/runs/{id}/abort`) added to the F9 OpenAPI contract and F9 T05/T06 | §17 |
| C-8 | Ownership rule amended to a **whitelist**: F2→`raw_postings` (delete), F3→`companies.stage`, F8→`companies.stage`/`employee_band` are reviewed seams; F2 SAD §2 corrected | §1 |
| C-9 | All six stale cost figures corrected to `PricingTable` ground truth; the "$2.16 is under $2.00" sentence fixed to "over → aborted pre-submission" | §3 |

### High (C-10…C-20) — all resolved

| # | Resolution | Decisions § |
|---|---|---|
| C-10 | `sub` claim now enforced on **both** `read` and `admin` policies (ADR-0014) | §9 |
| C-11 | Runbook R1 paths de-`/admin`-ed to match F9 contract; R4 SQL uses `started_at` | §17 |
| C-12 | F1 SAD `DO NOTHING` → `DO UPDATE SET last_seen_at` | §14 |
| C-13 | `CostAborted` made terminal; `--> Reporting` edge removed; reporting discharged by `RunCostAborted` handler; resume index correctly excludes it | §13 |
| C-14 | Every command assigned one owner; `/stats` retained in F10 catalogue | §8 |
| C-15 | F9→F6 and F9→F7 build-order edges added (F6/F7 endpoints depend on F9 T04) | §7, F6/F7 trackers |
| C-16 | All 11 epic estimates corrected to their tracker sums | §18 |
| C-17 | F4 count fixed (13), README grand total re-summed to **125** | §18 |
| C-18 | Ollama fallback adapter added: **F3 T14** | §17 |
| C-19 | Event catalog: `OccurredAt` on all payloads, `JobIndexRequested` publisher fixed, queue-naming generalised, `JobDuplicateDetected` key corrected, §2 diagram fixed | §10 |
| C-20 | Architecture-rule count settled to **8**, enumerated once in `coding-standards.md` §2 | §11 |

### Medium / Low (C-21…C-31, D-21…D-30) — all resolved

`JobHunter.TestKit` added to project lists (C-21) · crash matrix settled at 8 (C-22) · ERD
`}o--||` (C-23) · D11 acknowledges the `.uk.md` tier (C-24) · ADR-0005 O13→O5 (C-25) · F10 `/cv`
read-only metadata (C-26) · F5 AC-11 present-with-flag-on-timeout / suppress-on-unreachable (C-27) ·
near-duplicate grouping owned by F5 only (C-28) · F0 T13/T14 deps aligned to the graph (C-29) ·
instrument count 8 (C-30) · 22 commands everywhere (C-31). D-22 `Application` state diagram added ·
D-25 scope glossary published · D-26 £/$ rate stated · D-27 mis-citation removed · D-28 `D47` ordering
fixed · D-29/D-30 Npgsql API + identity comment corrected · shared artifact-scanner harness specified.

Remaining **conditions** tracked against M1 (documentation is complete; these are implementation-time
verifications, not doc defects): the genuinely-open O2 and O5 decisions must be answered before F9 T04
and F7 T07 respectively; frontmatter on the 120 task files (D-21) is a nice-to-have.
...
> **Superseded by the §0 Resolution Log.** The original verdict and its required-before-implementation
> list are preserved below for the record. As of the remediation, all items 1–15 are complete and the
> verdict is **✅ APPROVED WITH CONDITIONS** — the only outstanding conditions are answering the two
> genuinely-open decisions (O2, O5) before their blocked tasks (F9 T04, F7 T07).

# ❌ REJECTED *(original — now superseded)*
