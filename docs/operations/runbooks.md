...
|---|---|---|
| `Delivered` | Digest sent; Telegram or the network dropped it | Re-deliver: `POST /api/admin/runs/{id}/redeliver`. `delivery_log` prevents duplicates (invariant 8) |
...
| `Failed` | Read `failure_reason` | Fix the cause, then `POST /api/admin/runs/{id}/resume` |
...
kubectl exec -n infra-production $PGPOD -- psql -U postgres -d production_jobhunter -c \
  "SELECT s.ats_kind, count(*) FILTER (WHERE l.http_status BETWEEN 200 AND 299) AS ok,
    ...
    WHERE l.fetched_at > now() - interval '6 hours'
    GROUP BY s.ats_kind;"
```
