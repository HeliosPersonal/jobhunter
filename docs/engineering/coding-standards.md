...
| Rule | Rationale |
|---|---|
| `JobHunter.Domain` references no package except `Microsoft.Extensions.*.Abstractions` | The domain must be testable and portable; a domain that needs EF Core is not a domain |
| Dependency direction is `Hosts → Infrastructure/Claude/Scrapers/Search → Application → Domain` | [[../00-overview/sad]] §5 |
| `JobHunter.Contracts` references nothing | Events must be consumable by any host without dragging a dependency graph |
| No type in `Persistence/Queries/` calls `ExecuteAsync` or `Execute` | Dapper never writes ([[../00-overview/adr/0003-postgresql-efcore-dapper\|ADR-0003]]) |
| No use of `DateTime.Now`, `DateTime.UtcNow`, `DateTimeOffset.Now` outside `SystemClock` | Time must be injectable or nothing time-dependent is testable |
| Nothing outside `src/Aspire/` references `JobHunter.AppHost` | [[../00-overview/adr/0013-aspire-local-dev-only\|ADR-0013]] |
| Every `IEntityTypeConfiguration<T>` is `internal sealed` | Configuration is not API |
| No `public` type in `Infrastructure` that is not an extension method or an options class | Adapters are reached through ports, not directly |
