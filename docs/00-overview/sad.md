...
| S4 | **Two-tier Claude cascade on the Batch API, with a hard pre-submission cost ceiling** | 50% batch discount plus cheap-tier triage keeps a Run under $0.50; the ceiling makes QG-3 enforceable rather than aspirational. | [[adr/0005-anthropic-message-batches-two-tier-cascade\|0005]] |
...
sequenceDiagram
  ...
  O->>DB: create Run{state=Discovering, ceilingUsd}
...
- **How verify:** chaos integration test that kills the worker at each of six checkpoints and asserts final-state equality plus `delivery_log` uniqueness; outbox invariants asserted against a real Postgres via Testcontainers.
